...
<div class="page">
  ...
  <section>
    ...
    <div class="with-aside">
      ...
      <div class="aside">
        <span class="k">Where you can stop</span>
...
function buildSigNarrative() {
  // One line per (object, method): the smallest N you could have shot instead. Written from
  // the pairwise CIs, not from where the curve happens to look flat.
  const short = { colmap: 'COLMAP', mast3r_ga: 'MASt3R-GA', mast3r_ga_logwin7: 'MASt3R-GA logwin-7' };
  let rowsHtml = '';
  for (const obj of DATA.objects) {
    const objName = obj.title.replace(/ \(.*\)$/, '').replace('information_sign_002', 'sign').replace('bollard_003', 'bollard');
    const methods = [...new Set(obj.panels.map(k => panelState[k].d.method))];
    for (const method of methods) {
      ...
      if (plateau === first) {
        verdict = `<b>N=${first}</b>`;
        detail = 'already enough — nothing above it is resolvable';
      } else if (plateau) {
        const gain = rows.filter(r => r.n_lo === first && r.n_hi === plateau).map(r => r.delta)[0];
        verdict = `<b>N=${plateau}</b>`;
        detail = `worth ${gain > 0 ? '+' : ''}${gain.toFixed(0)}&nbsp;pts to get there, nothing after`;
      } else if (drops.length) {
        // best count is the one the last resolvable DROP starts from: it gained up to there
        // (sometimes only a little), and going past it measurably loses.
        const stopN = drops[drops.length - 1].n_lo;
        const up = rows.find(r => r.n_lo === first && r.n_hi === stopN);
        verdict = `<b>N=${stopN}</b>`;
        detail = (up && !up.includes_zero ? `+${up.delta.toFixed(1)}&nbsp;pts over N=${first}, and ` : '')
          + `going further costs ${Math.abs(drops[drops.length - 1].delta).toFixed(1)}&nbsp;pts`;
      } else {
        verdict = `<b>&gt;${last}</b>`;
        detail = `still gaining at the largest tested (+${(lastStep ? lastStep.delta : 0).toFixed(0)}&nbsp;pts on the last step)`;
      }
      rowsHtml += `<div style="margin-bottom:7px;">${short[method] || method} · ${objName}: ${verdict}`
        + `<div style="color:var(--text-faint); font-size:11.5px;">${detail}</div></div>`;
    }
  }
  return rowsHtml
    + `<div style="border-top:1px solid var(--panel-border); padding-top:7px; margin-top:2px;">`
    + `<b>The photos are for COLMAP.</b> It starts low and every batch buys real points — `
    + `+11 on the sign from 25 to 50, +39 on the bollard from 15 to 30, and on the bollard it has `
    + `still not finished at 60. MASt3R-GA is near its ceiling from the smallest count tested and `
    + `moves in single points after that: nothing above N=15 on the bollard, a peak at N=75 on the `
    + `sign (+2.5, logwin-7 +5.3) and then <b>2–3 points lost</b> at N=100, where the extra frames `
    + `add low-overlap pairs rather than coverage.</div>`;
}
