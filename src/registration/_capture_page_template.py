...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  @media (min-width:1040px) { #diff-hist-grid { grid-template-columns:repeat(2, minmax(0,1fr)) !important; } }
...
<div class="page">
  __SITE_NAV__
  <div>
    <div class="eyebrow">Capture-strategy ablation · exp_081–092 · gap-aware Chamfer (Accuracy / Completeness / F1)</div>
    ...
    <div style="font-size:11.5px; color:var(--text-faint); margin-top:4px;">
      Reconstructions <b>exp_081–092</b> · this page is generated by
      <span class="mono">src/registration/build_capture_comparison_page.py</span>
    </div>
  </div>
  ...
  <section>
    ...
    <div class="with-aside">
      <div>
        ...
        <div id="table-mode-note" style="font-size:11.5px; color:var(--text-faint); margin-top:8px;"></div>
      </div>
      <div class="aside">
  ...
  <section>
    ...
    <div class="with-aside">
      <div class="panel">
        <div id="f1-legend" style="font-size:10.5px; text-align:center; margin:2px 0 4px; color:var(--text-dim);"></div>
        <svg id="f1-chart" viewBox="0 0 1000 318" style="width:100%; height:318px;"></svg>
      </div>
      <div class="aside">
  ...
  <section>
    ...
    <div class="subtitle" style="max-width:92ch">
      ...
      each pair of approaches is re-scored 2000 times on resampled 5&nbsp;cm patches of the object. One row
      per pair: the dot is the difference, the bar is its 95%&nbsp;CI, the dashed line is 0. <b>If the bar
      sits entirely to one side of 0</b>, the approaches genuinely differ. <b>If it crosses 0</b>, the
      ranking is noise and they are indistinguishable at this sample size. Computed at 3&nbsp;cm.
      <br><br>
      The difference here is <b>unpaired</b>, and the bars are wider than they look on the frame-count
      page for that reason: two capture approaches are two separate walks around the object, so the two
      clouds share no frames and each side has to be resampled on its own. The nested frame sets on
      <a href="frame_count_study.html">frame_count_study.html</a> can be <i>paired</i> — the same
      resampled patches feed both sides, so the patch-to-patch variation cancels and only the effect
      remains. Pairing is the stronger test where the design allows it; this design does not.
    </div>
    <div class="with-aside">
      <div>
        ...
        <!-- 2 columns, so the four panels sit 2x2 instead of 3+1 -->
        <div id="diff-hist-grid" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(330px,1fr)); gap:12px;"></div>
        <div style="font-size:10.5px; color:var(--text-faint); text-align:center; margin-top:8px;">
          Each row is <b>left approach &minus; right approach</b>:
          <span class="swatch" style="background:var(--best)"></span>the left one scored higher&nbsp;&nbsp;
          <span class="swatch" style="background:var(--red)"></span>the right one did&nbsp;&nbsp;
...
// What the reader is shown. The browser holds only a capped subsample of each cloud
// (EMBED_CAP points per pool), and re-running DBSCAN on a thinned cloud finds fewer
// clusters, so the live gap mask UNDER-excludes and Accuracy reads low - on the sign's
// MASt3R T2 panel by 14 points. Whenever the configuration is one Python actually scored on
// the full cloud (this object's default gap settings, 3/5/10 cm), show Python's exact
// numbers: the ones in docs/tables/capture_comparison_summary.xlsx, and the ones the
// bootstrap CIs below belong to. The live estimate is for exploring with the tuner.
function atDefaults(objId) {
  const t = objTuner[objId], d = DATA.objects.find(o => o.id === objId).dbscan;
  return !!t && t.applyDbscan && t.ft === d.ft && t.eps === d.eps && t.mp === d.mp;
}
...
function buildPanel(key) {
  ...
  // View state (tab / colouring / overlay / threshold) is per object, not per panel: six
  // panels each carried an identical control strip and were in the same state anyway.
...
function refreshRowNotes() {
  for (const obj of DATA.objects) {
    ...
    for (const method of methods) {
      ...
      if (!el) continue;
      const f1 = APPROACHES.map(ap => shownMetrics(`${obj.id}__${method}__${ap}`, activeThreshold).f1);
      const spread = Math.max(...f1) - Math.min(...f1);
      const worst = APPROACHES[f1.indexOf(Math.min(...f1))];
      const wide = shownMetrics(`${obj.id}__${method}__${worst}`, 10.0).f1;
      const tight = shownMetrics(`${obj.id}__${method}__${worst}`, 3.0).f1;
...
function updateTable() {
  ...
  // Say which numbers are on screen - exact (tuner at defaults) or the browser's estimate.
  ...
  if (note) {
    ...
    note.innerHTML = anyEstimate
      ...
      : `Exact figures, computed on the full clouds — the same numbers as `
        + `<span class="mono">docs/tables/capture_comparison_summary.xlsx</span>. Move the tuner and the table `
  ...
  for (const obj of DATA.objects) {
    ...
    for (const method of methods) {
      ...
      const rows=[];
      for (const ap of APPROACHES) {
        const key=`${obj.id}__${method}__${ap}`; const m=shownMetrics(key,activeThreshold);
...
function updateChart() {
  ...
  const W=1000,H=318,padL=42,padR=12,padT=20,padB=42;
  ...
  groups.forEach((g,gi)=>{
    ...
    APPROACHES.forEach((ap,ai)=>{
      const key=`${g.obj}__${g.method}__${ap}`; const m=shownMetrics(key,activeThreshold);
      ...
      const dF1 = shownMetrics(key,10.0).f1 - shownMetrics(key,3.0).f1;
      s+=`<text x="${bx+barW/2}" y="${by-15}" font-size="8.5" fill="${tick}" fill-opacity="0.75" text-anchor="middle">Δ${dF1.toFixed(0)}</text>`;
    });
    s+=`<text x="${gx+gW/2}" y="${padT+plotH+22}" font-size="10.5" fill="${textc}" text-anchor="middle">${g.label}</text>`;
    ...
    const f1s = APPROACHES.map(ap => shownMetrics(`${g.obj}__${g.method}__${ap}`, activeThreshold).f1);
    ...
    s+=`<text x="${gx+gW/2}" y="${padT+plotH+36}" font-size="9.5" fill="${tick}" fill-opacity="0.85" text-anchor="middle">${verdict}</text>`;
  });
  svg.innerHTML=s;

  // Legend lives in HTML above the plot rather than inside the SVG: centred, and it wraps
  // on narrow screens instead of running its labels into each other.
  const leg = document.getElementById('f1-legend');
  if (leg) leg.innerHTML = APPROACHES.map(ap =>
    `<span style="white-space:nowrap; margin:0 12px;">`
    + `<span style="display:inline-block; width:11px; height:11px; border-radius:3px; vertical-align:-1px;`
    + ` background:${cssvar(APPROACH_COLORVAR[ap])}; opacity:.8;"></span>&nbsp;${DATA.approach_label[ap]}</span>`
  ).join('');
}
...
// ---------- pairwise-difference panels (object x method) ----------
// Ported from frame_count_study.html, which asks the same question and answers it in a
// forest plot: one row per pair, dot at the difference, bar across its 95% CI, dashed line
// at 0. This section used to overlay three translucent histograms per panel; on the sign the
// distributions sit so far apart that they were three thin spikes in an empty field - a shape
// that said nothing the number beside it did not already say.
const SIG_COLORS = { pos: '--best', neg: '--red', ns: '--text-faint' };

// Shared x domain per object, so an interval drawn twice as wide really is twice as wide.
function sigDomain(objId) {
  const all = DATA.sensitivity.filter(e => e.object === objId).flatMap(e => e.pairwise);
...
function renderSigPanel(sv) {
  const obj = DATA.objects.find(o => o.id === sv.object);
  const rows = sv.pairwise;
  const W = 452, padL = 76, padR = 112, padT = 20, padB = 30, rowH = 21;
  const H = padT + rows.length * rowH + padB;
  const plotW = W - padL - padR;
  const { dLo, dHi } = sigDomain(sv.object);
  ...
  rows.forEach((r, i) => {
    ...
    const col = cssvar(sig ? (r.delta > 0 ? SIG_COLORS.pos : SIG_COLORS.neg) : SIG_COLORS.ns);
    ...
    s += `<circle cx="${X(r.delta).toFixed(1)}" cy="${y}" r="3.4" fill="${col}" fill-opacity="${sig ? 1 : 0.6}">`
      + `<title>${r.label}: ${r.delta > 0 ? '+' : ''}${r.delta.toFixed(2)} pp, 95% CI [${r.ci_lo.toFixed(2)}, ${r.ci_hi.toFixed(2)}]</title></circle>`;
    s += `<text x="${padL - 8}" y="${y + 3.2}" font-size="9.5" fill="${dim}" text-anchor="end">${r.label}</text>`;
  ...
  s += `<text x="${(padL + plotW / 2).toFixed(1)}" y="${H - 3}" font-size="8.5" fill="${tick}" text-anchor="middle">`
    + `&Delta; F1@3cm (pp) &middot; ${DATA.bootstrap.n_draws} bootstrap draws</text>`;
