...
function renderDiffHists() {
  ...
  for (const sv of DATA.sensitivity) {
    ...
    // One centred block above the plot instead of a legend above and figures below: the two
    // carried the same three labels, and at ~300px per panel the figures wrapped into a
    // cramped three-line run under the chart. Each row is now swatch + pair + delta + CI.
    const rows = sv.pairwise.map((p, pi) => {
      const sig = !p.includes_zero;
      return `<div style="white-space:nowrap; font-weight:${sig ? '650' : '400'}; color:${sig ? 'var(--text)' : 'var(--text-dim)'};">`
        + `<span style="display:inline-block; width:9px; height:9px; border-radius:2px; vertical-align:0;`
        + ` background:${PAIR_COLORS[pi]}; opacity:${sig ? '1' : '.65'};"></span>&nbsp;`
        + `${p.label}: ${p.delta > 0 ? '+' : ''}${p.delta.toFixed(1)} [${p.ci_lo.toFixed(1)}, ${p.ci_hi.toFixed(1)}]`
        + `${sig ? '&nbsp;*' : ''}</div>`;
    }).join('');
    const panel = document.createElement('div');
    panel.className = 'panel';
    ...
    panel.innerHTML = `<div style="font-weight:650; font-size:12.5px;">${obj.title} &middot; ${DATA.method_label[sv.method]}</div>`
      + `<div style="font-size:10.5px; line-height:1.5; text-align:center; margin:4px 0 2px;">${rows}`
      + `<div style="color:var(--text-faint); font-size:9.5px; margin-top:1px;">* = 95% CI excludes 0</div></div>`
